...
const calculate = () => {
  ...
  if (selectedItems.indexOf("--Select--") !== -1) {
    alert(
      "Please select an ingredient in each input or reduce the Number of ingredients",
    );
    return 1;
  } else {
    const arrays = selectedItems.map(
      (itemName) => itemObj[itemName]["nutrientArr"],
    );
    const ingredientArray = selectedItems.map(
      (itemName) => itemObj[itemName]["ingredientsArr"],
    );
